...
class MainVisualizer:
  ...
  def plot_cont_validation_eval(self, task_data, **kwargs):
    ...
    fig, axs = plt.subplots(
      len(task_data), sharex=True, sharey=True, figsize=(10, len(task_data) * 2)
    )
    if len(task_data) == 1:
      axs = [axs]
    plt.subplots_adjust(
      left=0.125, right=0.9, bottom=0.1, top=1, wspace=0.2, hspace=0.8
    )
    for nr, task in enumerate(task_data):
      ...
      for j, i in enumerate(task["val_task_results"]):
        ...
        axs[nr].plot(
          i[0], i[1], color=val, linestyle=line_styles[j], label=task["eval_names"][j]
        )
  ...
  def __init__(self, p_visu, logger=None, epoch=0, store=True, num_classes=22):
    ...
    if num_classes == 41:
      self.SEG_COLORS = np.array(
        [(*SCANNET_COLOR_MAP[k], 255) for k in SCANNET_COLOR_MAP.keys()], dtype=np.uint8
      )
  ...
  def plot_nyu_confusion_matrix(self, conf, title="NYU40 Confusion Matrix", **kwargs):
    ...
    for i in range(len(label_y)):
      ...
      rect = patches.Rectangle(
        (i - 0.5, -0.5), 1, 1, linewidth=1, edgecolor=None, facecolor=vals
      )
      rect2 = patches.Rectangle(
        (0 - 0.5, i - 0.5), 1, 1, linewidth=1, edgecolor=None, facecolor=vals
      )
      ax.add_patch(rect)
      ax.add_patch(rect2)
...
def test():
  ...
  visu = Visualizer(
    os.getenv("HOME") + "/tmp", logger=None, epoch=0, store=False, num_classes=41
  )
